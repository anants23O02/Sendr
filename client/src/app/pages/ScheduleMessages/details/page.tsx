        ...
        <main className="flex-1 p-4 sm:p-6 bg-gray-50 overflow-y-auto">
          <div className="max-w-5xl mx-auto bg-white shadow-md rounded-lg p-6">
            <div className="text-center mb-8">
              <h1 className="text-2xl sm:text-3xl font-bold text-gray-800">Message Details</h1>
              <p className="text-gray-600 mt-2 text-sm sm:text-base">
                Provide details of the message along with your scheduling preferences.
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-gray-100 p-4 rounded-lg">
              <div>
                ...
                <select id="messageType" className="mt-1 border border-gray-300 rounded-md p-2 w-full">
              ...
              <div>
                ...
                <Input id="recipientNumber" placeholder="Enter recipient number" maxLength={10} required />
              </div>

              <div>
                <Label htmlFor="startDate">Start Date</Label>
                <input type="date" id="startDate" className="mt-1 border rounded-md p-2 w-full" />
              </div>

              <div>
                <Label htmlFor="endDate">End Date</Label>
                <input type="date" id="endDate" className="mt-1 border rounded-md p-2 w-full" />
              </div>
              ...
              {sendFrequency === "Once" && (
                <div className="md:col-span-2">
                  ...
                  <input type="time" id="MessageTime" className="mt-1 border border-gray-300 rounded-md p-2 w-full" />
                </div>
              )}
                ...
                <div className="md:col-span-2">
                  ...
                  {multipleTimes.map((time, index) => (
                    <div key={index} className="flex items-center gap-3 mt-2">
                      ...
                      />
                      <button type="button" onClick={() => removeTimeField(index)} className="text-red-600 text-xl">×</button>
                    </div>
                  ))}
                  <button
                    type="button"
              ...
              <div className="mt-6">
                ...
                <div className="flex flex-wrap gap-2">
                  {weekdays.map((day) => (
                    <button
                      key={day}
                      onClick={() => handleToggle(selectedWeekdays, day, setSelectedWeekdays)}
            ...
            {messageCount === "Monthly" && (
              <div className="mt-6">
                <h2 className="text-lg font-semibold mb-2">Select Day(s) of the Month</h2>
                <div className="grid grid-cols-7 gap-2">
                  {Array.from({ length: 31 }, (_, i) => {
                    ...
                    return (
                      <button
                        key={day}
                        onClick={() => handleToggle(selectedMonthDays, day, setSelectedMonthDays)}
                        className={`px-3 py-1 text-sm border rounded ${
                          selectedMonthDays.includes(day) ? "bg-gray-800 text-white" : "bg-white text-gray-800"
                        }`}
                      >
                        {day}
                      </button>
            ...
            {messageCount === "Yearly" && (
              <div className="mt-6">
                <h2 className="text-lg font-semibold mb-4">Select Month and Dates</h2>
                    ...
                    <div key={month} className="border rounded p-4">
                      ...
                      <div className="grid grid-cols-7 gap-1">
                        {Array.from({ length: 31 }, (_, i) => {
                          ...
                          const isPast = new Date(`${month} ${i + 1}, ${today.getFullYear()}`) < today;

                          return (
                            <button
                              key={label}
                              onClick={() => handleToggle(selectedYearlyDates, label, setSelectedYearlyDates)}
                              disabled={isPast}
            ...
            {messageCount === "Custom" && (
              <div className="mt-6">
                <h2 className="text-lg font-semibold mb-2">Select Custom Dates</h2>
                <input
                  type="date"
                ...
                <div className="flex flex-wrap gap-2 mt-3">
                  {customDates.map((date) => (
                    <span key={date} className="bg-gray-200 px-3 py-1 rounded-full flex items-center">
                      {date}
                      <button onClick={() => handleCustomDateRemove(date)} className="ml-2 text-red-500">×</button>
                    </span>
                  ))}
                </div>
              </div>
